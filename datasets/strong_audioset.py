...
class StrongAudioSetDataset(TorchDataset):
    def __init__(self, wav_file, sample_rate=32000, resample_rate=32000, classes_num=527,
        ...
        with h5py.File(wav_file, 'r') as f:
            self.length = len(f['audio_name'])
            print(f"Dataset from {wav_file} with length {self.length}.")
    ...
    def __getitem__(self, index):
        ...
        audio_name = audio_name.replace(".wav", "").split("Y", 1)[1]
...
def get_base_training_set(resample_rate=32000, gain_augment=0):
    balanced_train_hdf5 = dataset_config['balanced_train_hdf5']
    ds = StrongAudioSetDataset(balanced_train_hdf5, resample_rate=resample_rate, gain_augment=gain_augment)
    return ds


def get_unbalanced_training_set(resample_rate=32000, gain_augment=0):
    unbalanced_train_hdf5 = dataset_config['unbalanced_train_hdf5']
    ds = StrongAudioSetDataset(unbalanced_train_hdf5, resample_rate=resample_rate, gain_augment=gain_augment)
    return ds


def get_base_test_set(resample_rate=32000):
    eval_hdf5 = dataset_config['eval_hdf5']
    ds = StrongAudioSetDataset(eval_hdf5, resample_rate=resample_rate)
    return ds
